...
| `modifier` | 关键字 `modifier` / keyword `modifier` |
| `left_paren` | `(` |
| `right_paren` | `)` |
| `left_brace` | `{` |
| `right_brace` | `}` |
...
program 语法支持零个或多个顶层 declaration，后接一个最终 expression。
Program syntax supports zero or more top-level declarations followed by one
final expression.

`val` declaration 第一版必须以 `;` 结束。
The first `val` declaration form must end with `;`.

| Form / 形式 | Rule / 规则 |
| --- | --- |
| program / 程序 | declaration* expression |
| val declaration / val 声明 | `val` identifier `=` expression `;` |
| modifier declaration / modifier 声明 | `modifier` identifier `{` modifier-pattern `}` |
| raw modifier application / raw modifier application | identifier token-run-with-block |

expression 语法支持以下形式。
Expression grammar supports these forms.
...
| call expression / 调用表达式 | `identifier` `(` argument-list? `)` |
...
| `PARSE003` | 缺少必需 token，例如 `right_paren`、`identifier`、`equals` 或 `semicolon` / expected a required token such as `right_paren`, `identifier`, `equals`, or `semicolon` |
