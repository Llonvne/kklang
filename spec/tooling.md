...
DSL feature 名称固定为 `kk-file-type`、`syntax-highlighter`、`minimal-psi`、`diagnostic-annotator`、`idea-icon-assets`、`single-file-run-configuration`、`current-file-run-configuration-producer`、`current-file-run-line-marker`、`native-debug-command-hint` 和 `installable-plugin-zip`。
...
`current-file-run-line-marker`, `native-debug-command-hint`, and
