...
第一版工具链要求 `.kk` 文件识别、语法高亮、LSP 诊断、LSP semantic tokens、IDEA 最小 PSI、IDEA 诊断标注、IDEA 单文件运行配置和 IDEA 当前文件运行入口。
...
annotations, an IDEA single-file run configuration, and IDEA current-file run
entry points.
...
IDEA 插件必须提供 `.kk` 文件类型、syntax highlighter、最小 PSI shell、diagnostic annotator 和仓库内的 `kklang` 图标资产。
The IDEA plugin must provide the `.kk` file type, a syntax highlighter, a
minimal PSI shell, a diagnostic annotator, and the in-repository `kklang` icon
asset.

DSL feature 名称固定为 `kk-file-type`、`syntax-highlighter`、`minimal-psi`、`diagnostic-annotator`、`idea-icon-assets`、`single-file-run-configuration`、`current-file-run-configuration-producer`、`current-file-run-line-marker`、`native-debug-command-hint`、`native-debug-executor-launch` 和 `installable-plugin-zip`。
The fixed DSL feature names are `kk-file-type`, `syntax-highlighter`,
`minimal-psi`, `diagnostic-annotator`, `idea-icon-assets`,
`single-file-run-configuration`, `current-file-run-configuration-producer`,
`current-file-run-line-marker`, `native-debug-command-hint`,
`native-debug-executor-launch`, and `installable-plugin-zip`.

IDEA 图标必须来自仓库资源，不得从 Codex 生成目录或本机绝对路径加载；同一图标必须用于 `.kk` 文件类型和 `kklang` 单文件运行配置。
IDEA icons must come from repository resources and must not be loaded from the
Codex generated-image directory or a local absolute path; the same icon must be
used for the `.kk` file type and the `kklang` single-file run configuration.
...
IDEA 插件必须按 IntelliJ 标准单文件模式为当前 `.kk` 文件注册 run line marker；marker 必须复用当前文件的 run configuration producer，不得写死文件名或猜测默认入口文件。
The IDEA plugin must register a run line marker for the current `.kk` file using
IntelliJ's standard single-file mode; the marker must reuse the current-file run
configuration producer and must not hard-code file names or guess a default
entry file.

IDEA 单文件运行配置在文件位于 kklang 仓库内时，必须在 Run console 暴露 Native debug executable、构建命令和 LLDB 命令。
When the file is inside a kklang repository, the IDEA single-file run
configuration must expose the Native debug executable, build command, and LLDB
command in the Run console.

IDEA Native debug 命令必须通过仓库根目录标记生成；仓库根目录标记是 `settings.gradle.kts` 文件和 `runtime/kn` 目录同时存在。
The IDEA Native debug command must be generated from repository-root markers;
the repository-root markers are the presence of `settings.gradle.kts` and the
`runtime/kn` directory together.

IDEA 单文件运行配置在 Debug executor 下必须启动仓库内 Native debug executable 的 LLDB 会话；启动前必须执行 `:runtime:kn:linkKkrunDebugExecutableHost`，并把当前 `.kk` 文件路径作为 executable 参数传入。
Under the Debug executor, the IDEA single-file run configuration must start an
LLDB session for the in-repository Native debug executable; before launch it must
run `:runtime:kn:linkKkrunDebugExecutableHost` and pass the current `.kk` file
path as the executable argument.

IDEA Debug console 必须显示 Native build 命令、LLDB launch 命令和 C runtime 断点提示，并允许用户输入 LLDB 命令。
The IDEA Debug console must display the Native build command, the LLDB launch
command, and a C-runtime breakpoint hint, and it must allow the user to enter
LLDB commands.
